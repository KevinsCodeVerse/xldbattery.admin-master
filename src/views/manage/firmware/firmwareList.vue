<template>
    <div>
        <main-head>
            <el-button type="primary" slot="after" class="top_btn" @click="visible = true">新建固件</el-button>
        </main-head>
        <main-content>
...
<!--                </el-table-column>-->
                <el-table-column label="操作" align="center" fixed="right" min-width="120px">
                    <template slot-scope="scope">
                        <el-button type="primary" size="small" @click="setData(scope.row)">编辑固件</el-button>
                        ...
                        <el-button v-if="scope.row.status==0" type="success" size="small" @click="issueShow = true,issFormData.id= scope.row.id">发布固件</el-button>
                        <el-button v-if="scope.row.status==1" type="warning" size="small" @click="issueEetData(scope.row)">编辑发布</el-button>
                        <el-button type="danger" size="small" @click="del(scope.row.id)">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>

            <el-pagination class="page" background :small="$store.state.isPhone" :layout="$store.state.isPhone?'total, prev, pager, next':'total, sizes, prev, pager, next, jumper'" :current-page.sync="params.pageNo" :page-size.sync="params.pageSize" :total="total" @size-change="search" @current-change="getList">
            </el-pagination>

            <el-dialog :title="formData.id ? '编辑' : '新增'" :visible.sync="visible" center width="600px" :before-close="handleClose" :close-on-click-modal="false">
            <el-form ref="formData" :model="formData" :rules="rules" size="medium" label-width="120px">
                ...
                <el-form-item label="上传固件" prop="file">
                 ...
                 <el-form-item label="固件大小" prop="wareSize">
                    <el-input v-model="formData.wareSize" disabled placeholder="请先选择文件再上传至服务器"/>
                </el-form-item>
                <el-form-item label="固件地址" prop="wareUrl">
                    <el-input v-model="formData.wareUrl" disabled placeholder="请先选择文件再上传至服务器"/>
                </el-form-item>
                <el-form-item label="硬件版本" prop="hardVersion">
                    <el-input v-model="formData.hardVersion" placeholder="请输入硬件版本"/>
                </el-form-item>
                <el-form-item label="软件版本" prop="softVersion">
                    <el-input v-model="formData.softVersion" placeholder="请输入软件版本"/>
                </el-form-item>
            </el-form>
            <div slot="footer">
            ...
            <el-dialog :title="issFormData.id ? '编辑' : '新增'" :visible.sync="issueShow" center width="600px" :before-close="handleIssueClose" :close-on-click-modal="false">
            <el-form ref="issFormData" :model="issFormData" :rules="issueRules" size="medium" label-width="120px">
                <el-form-item label="设备序列号" prop="deviceCode">
                    <el-input v-model="issFormData.deviceCode" placeholder="请输入设备序列号，支持模糊和精确匹配"/>
                </el-form-item>
                    <div class="hint_text">注:匹配序列号内包含以上连续字段</div>
                
                <el-form-item label="软硬件版本号" prop="firmwareVersionList">
                    <el-button size="small" type="primary" @click="addItem">添加目标软硬件版本号</el-button>
                </el-form-item>
                <div v-for="(item,index) in versionList" :key="index">
                     <el-button icon="el-icon-minus" @click="deleteItem(item,index)"
                       ...
                       <el-form :model="item" ref="form" :rules="issueRules" label-width="80px" :inline="false" size="normal">
                           <el-form-item label="目标软件版本号" label-width="165px" prop="reSoftVersion">
                        <el-input v-model="item.reSoftVersion" placeholder="请输入要升级的8位软件版本号"/>
                    </el-form-item>
                    <el-form-item label="目标硬件版本号" label-width="165px" prop="reHardVersion">
                        <el-input v-model="item.reHardVersion" placeholder="请输入要升级的8位硬件版本号"/>
                    </el-form-item>
                       </el-form>
            ...
            <div slot="footer">
                <el-button @click="clearFormData(), issueShow = false">关闭</el-button>
                <el-button type="primary" @click="issueAddOrUpdate" :loading="formLoading">确定</el-button>
            </div>
            </el-dialog>
        </main-content>
    </div>
    ...
    methods:{
        ...
        issueAddOrUpdate(){
            ...
            for (let i = 0; i <  this.versionList.length; i++) {
                if(this.versionList[i].reHardVersion=='' || this.versionList[i].reSoftVersion==''){
                    return this.$message.warning('请补充完整软硬件版本号')
                }
            }
            this.issFormData.firmwareVersionList = JSON.stringify([...this.versionList])
        ...
        submitUpload(){
            if(!this.file) return this.$message.warning('请先上传文件至服务器再点击')
